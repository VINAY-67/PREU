...
const CreateCommunityModal = ({ isVisible, onClose, onSuccess, Button, Input }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setTimeout(() => {
            ...
            // Clear data from local storage on successful submission
            localStorage.removeItem(STORAGE_KEY);
            
            // Trigger success state in parent (App)
            onSuccess(); 
        }, 2000);
    };
...
//here i need to check to send this to the admin dahsboard 
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <Button 
                                    ...
                                    disabled={submitting || !formData.agreed || formData.proposal.length < 50} // Ensure this logic is present
                                >
                                    {submitting ? (
                                        <>
...
const CommunityItem = ({ community, handleJoin, joining }) => {
...
const [isToastVisible, setIsToastVisible] = useState(false);
  ...
  const handleJoinCommunity = async (communityId) => {
    ...
    setError("");
    try {
      await new Promise(resolve => setTimeout(resolve, 1000)); // Simulate joining delay
  ...
  return (
    <div className={`${NEUMO_BG} text-gray-800 min-h-screen font-sans`}>
      ...
      <Toast
        message="Proposal Submitted! Admins will review the details.and let you now"
